...
class ChargeRules extends Component {
    ...
    //跳转详情
    handleToDetail(id) {
        window.location.hash = `${location.hash}/DisplayChargeRules?id=` + id;
    }

    //跳转编辑
    handleToEdit(id) {
        window.location.hash = `${location.hash}/EditChargeRules?id=` + id;
    }

    //跳转新增
    handleToInsert() {
        if (window.checkPageEnable("chargeRuleAdd")) {
            window.location.hash = `${location.hash}/InsertChargeRules`;
        } else {
