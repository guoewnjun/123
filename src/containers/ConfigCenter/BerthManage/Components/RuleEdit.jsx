...
import RuleInsert from './RuleInsert.jsx';

import {HttpClient} from "@/common/HttpClient";
import {custom} from "@/common/SystemStyle";
import {Global} from "@/common/SystemFunction";

import {Button, message, Radio, Card, Input, Form, Tooltip, Icon} from "antd";
...
class RuleEdit extends Component {
    constructor (props) {
        ...
        this.state = {
            workdayDaytimeValue: this.props.rule.workDayPriceRuleVO.daytimeType,
            workdayNighttimeValue: this.props.rule.workDayPriceRuleVO.nighttimeType,
            weekendDaytimeValue: this.props.rule.weekendPriceRuleVO.daytimeType,
            weekendNighttimeValue: this.props.rule.weekendPriceRuleVO.nighttimeType,

            workdayTop: this.props.rule.workDayPriceRuleVO.upperLimitPrice,
            weekendTop: this.props.rule.weekendPriceRuleVO.upperLimitPrice,
            workdayDaytimeForFree: this.props.rule.workDayPriceRuleVO.daytimeFreeMin,
            workdayNighttimeForFree: this.props.rule.workDayPriceRuleVO.nighttimeFreeMin,
            weekendDaytimeForFree: this.props.rule.weekendPriceRuleVO.daytimeFreeMin,
            weekendNighttimeForFree: this.props.rule.weekendPriceRuleVO.nighttimeFreeMin,

            name: this.props.rule.name,
            number: this.props.rule.number,


            workdayDaytimeValueHadChange: false,
            workdayNighttimeValueHadChange: false,
            weekendDaytimeValueHadChange: false,
            weekendNighttimeValueHadChange: false,

            inSubmit: false,

            freeTimeChargeType: this.props.rule.freeTimeChargeType,
            workDaySingleDayChargingInterval: this.props.rule.workDayPriceRuleVO.chargeTimeType || 3,
            restDaySingleDayChargingInterval: this.props.rule.weekendPriceRuleVO.chargeTimeType || 3,

            initialWorkDayChargeType: this.props.rule.workDayPriceRuleVO.chargeTimeType,
            initialWeekendChargeType: this.props.rule.weekendPriceRuleVO.chargeTimeType,

            isWorkdayAlike: this.props.rule.isWorkdayAlike, // true表示与工作日一致
        }
    }
    ...
    componentDidMount () {
        //默认初始值
        this.props.form.setFieldsValue({ ruleName: this.state.name });
        this.props.form.setFieldsValue({ ruleNumber: this.state.number });
        // this.props.form.setFieldsValue({ isWorkdayAlike: this.props.rule.isWorkdayAlike ? 1 : 2 });

        this.props.form.setFieldsValue({ workdayTop: this.state.workdayTop });
        this.props.form.setFieldsValue({ workdayDaytimeForFree: this.state.workdayDaytimeForFree });
        this.props.form.setFieldsValue({ workdayNighttimeForFree: this.state.workdayNighttimeForFree });
        // this.props.form.setFieldsValue({ weekendTop: this.state.weekendTop });
        // this.props.form.setFieldsValue({ weekendDaytimeForFree: this.state.weekendDaytimeForFree });
        // this.props.form.setFieldsValue({ weekendNighttimeForFree: this.state.weekendNighttimeForFree });
        setTimeout(
            () => {
    ...
    onChange (key, e) {
        console.log("key", key, 'radio checked', e.target.value);
        this.setState({
            [key]: e.target.value,
            [key + "HadChange"]: true
        });
    ...
    submit () {
        ...
        const customRule = !this.state.isWorkdayAlike ? (((weekendDaytimeSmallCar) || (weekendNighttimeSmallCar))) : true;
        if (
            ...
            const workDayPriceRuleVO = {
                id: this.props.rule.workDayPriceRuleVO.id,
            ...
            if (!this.state.isWorkdayAlike) {
                weekendPriceRuleVO = {
                    id: this.props.rule.weekendPriceRuleVO.id,
            ...
            let params = {
                ...
                isWorkdayAlike: this.state.isWorkdayAlike,  // 是否与工作日保持一致
                freeTimeChargeType: this.state.freeTimeChargeType, // 免费停车是否需要计费
                workDayPriceRuleVO: this.filterParams(workDayPriceRuleVO),
            };
            if (!this.state.isWorkdayAlike) params.weekendPriceRuleVO = this.filterParams(weekendPriceRuleVO);
            console.log('编辑计费规则提交参数：', params);
            // message.info('现暂时没有调用提交接口，请先在控制台中查看提交参数');
            // return;
            this.state.inSubmit = true;
            HttpClient.query('/parking-resource/parkingPriceRules/' + this.props.rule.id, HttpClient.PUT, JSON.stringify(params), this.fetchEdit.bind(this));
    ...
    onSingleDayChargingIntervalChange (key, e) {
        ...
        if (key === 'workDay') {
            this.setState({
                ...
                initialWorkDayChargeType: 0, // 不是初始化了
            }, () => {
                // 以下为了防止设置表单值出错
                setTimeout(() => {
            ...
            this.setState({
                ...
                initialWeekendChargeType: 0, // 不是初始化了
            }, () => {
                setTimeout(() => {
                    this.props.form.setFieldsValue({
    ...
    isCustomRuleChange (e) {
        this.setState({
            isWorkdayAlike: e.target.value === 1,
        });
    ...
    render () {
        ...
        const { workDaySingleDayChargingInterval, restDaySingleDayChargingInterval, isWorkdayAlike, freeTimeChargeType, initialWorkDayChargeType, initialWeekendChargeType } = this.state;
        const popoverInfo = (
            ...
            <div className="page-content" style={{ backgroundColor: "rgba(0,0,0,0)", padding: 0 }}>
                    ...
                    <Card title='规则详情'>
                        <div style={{ float: "left" }}>
                            ...
                            <FormItem>
                                {getFieldDecorator("ruleName", {
                                    ...
                                    <Input className="detail_single_input" style={{ width: 326, marginBottom: 0 }}
                                           placeholder="请输入" suffix={this.state.name.length + "/20"} onChange={(e) => {
                        ...
                        <FormItem style={{ marginBottom: 0 }}>
                            {getFieldDecorator("freeTimeChargeType", {
                                ...
                                initialValue: freeTimeChargeType || null
                            })(
                ...
                <Card title="工作日计费规则">
                    <Form>
                        {/*封顶价*/}
                        <div style={{ float: 'left' }}>
                            <div className="detail_display_sub_title"><span className="require_status">*</span>封顶价</div>
                            <FormItem>
                                {getFieldDecorator("workdayTop", {
                                    ...
                                    <Input className="detail_single_input" style={{ width: 326 }} placeholder="请输入"
                                           suffix="元"
                                           onChange={(e) => this.state.workdayTop = e.target.value}/>
                                )}
                            </FormItem>
                        </div>
                        {/*单日计费区间*/}
                        <div style={{ marginLeft: "47px", float: "left" }}>
                            ...
                            <FormItem>
                                {getFieldDecorator('workSingleDayChargeRadio', {
                                    ...
                                    initialValue: workDaySingleDayChargingInterval
                                })(
                        ...
                        <div id='workdayDayAndNight'>
                            <div
                                ...
                                <div style={{ marginTop: "24px", marginBottom: "16px", width: "100%" }}>
                                    <div className="detail_display_title" style={{ float: "left" }}>白天设置</div>
                                ...
                                {
                                    (initialWorkDayChargeType === 0 || initialWorkDayChargeType === 1 || initialWorkDayChargeType === 3) ? (
                                        <Fragment>
                                            <RuleInsert
                                                ...
                                                dayType="workday"
                                                timeType="daytime"
                                                ruleDetailList={this.state.workdayDaytimeValueHadChange ? undefined : this.props.rule.workDayPriceRuleVO.daytimePriceRuleDayWayList}
                                            />
                                            {/*<RuleInsert
                                                ...
                                                dayType="workday"
                                                timeType="daytime"
                                                ruleDetailList={this.state.workdayDaytimeValueHadChange ? undefined : this.props.rule.workDayPriceRuleVO.bigCarDaytimePriceTempleteList}
                                            />*/}
                                        </Fragment>
                                    ) : ''
                                }
                            ...
                            <div
                                ...
                                {/*夜间设置*/}
                                <div style={{ marginTop: "24px", marginBottom: "16px", width: "100%" }}>
                                    <div className="detail_display_title" style={{ float: "left" }}>夜间设置</div>
                                    <div className="detail_display_content" style={{
                                        float: "left",
                                        marginLeft: "8px",
                                        background: "#E8E8E8",
                                ...
                                {
                                    (initialWorkDayChargeType === 0 || initialWorkDayChargeType === 2 || initialWorkDayChargeType === 3) ? (
                                        <Fragment>
                                            <RuleInsert
                                                ...
                                                dayType="workday"
                                                timeType="nighttime"
                                                ruleDetailList={this.state.workdayNighttimeValueHadChange ? undefined : this.props.rule.workDayPriceRuleVO.nighttimePriceRuleDayWayList}
                                            />
                                            {/*<RuleInsert
                                                ...
                                                dayType="workday"
                                                timeType="nighttime"
                                                ruleDetailList={this.state.workdayNighttimeValueHadChange ? undefined : this.props.rule.workDayPriceRuleVO.bigCarNighttimePriceTempleteList}
                                            />*/}
                                        </Fragment>
                                    ) : ''
                                }
                ...
                <Card title="非工作日计费规则">
                    ...
                    <Form>
                        <FormItem>
                            {getFieldDecorator('isWorkdayAlike', {
                                initialValue: isWorkdayAlike ? 1 : 2
                            })(
                                <RadioGroup className="detail_single_input"
                                        ...
                                        <div style={{ marginLeft: "47px", float: "left" }}>
                                            ...
                                            <FormItem>
                                                {getFieldDecorator('restSingleDayChargeRadio', {
                                                    ...
                                                    initialValue: restDaySingleDayChargingInterval
                                                })(
                                                    <RadioGroup className="detail_single_input"
                        ...
                        !isWorkdayAlike && (
                            <div id='weekendDayAndNight'>
                                <div
                                    ...
                                    {
                                        (initialWeekendChargeType === 0 || initialWeekendChargeType === 1 || initialWeekendChargeType === 3) ? (
                                            <Fragment>
                                                <RuleInsert
                                                    ...
                                                    dayType="weekend"
                                                    timeType="daytime"
                                                    ruleDetailList={this.state.weekendDaytimeValueHadChange ? undefined : this.props.rule.weekendPriceRuleVO.daytimePriceRuleDayWayList}
                                                />
                                                {/*<RuleInsert
                                                ...
                                                dayType="weekend"
                                                timeType="daytime"
                                                ruleDetailList={this.state.weekendDaytimeValueHadChange ? undefined : this.props.rule.weekendPriceRuleVO.bigCarDaytimePriceTempleteList}
                                            />*/}
                                            </Fragment>
                                        ) : ''
                                    }
                                ...
                                <div
                                    ...
                                    {
                                        (initialWeekendChargeType === 0 || initialWeekendChargeType === 2 || initialWeekendChargeType === 3) ? (
                                            <Fragment>
                                                <RuleInsert
                                                    ...
                                                    dayType="weekend"
                                                    timeType="nighttime"
                                                    ruleDetailList={this.state.weekendNighttimeValueHadChange ? undefined : this.props.rule.weekendPriceRuleVO.nighttimePriceRuleDayWayList}
                                                />
                                                {/*<RuleInsert
                                                ...
                                                dayType="weekend"
                                                timeType="nighttime"
                                                ruleDetailList={this.state.weekendNighttimeValueHadChange ? undefined : this.props.rule.weekendPriceRuleVO.bigCarNighttimePriceTempleteList}
                                            />*/}
                                            </Fragment>
                                        ) : ''
                                    }
...
export default Form.create()(RuleEdit);
