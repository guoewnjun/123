...
class OperationalWeekly extends Component {
    ...
    loadData(week) {
        ...
        HttpClientImmidIot.query('/containers/DataCenter/OperationalDaily', 'GET', weeks, this.handleQueryData.bind(this))
    }
    ...
    render() {
      ...
      const dataOff = [
        ...
        const {loading,week,userProfile,berthSituation,parkingProfile,spendingProfile,complaintsOverview,maintenanceOverview,stopOverview,patrolInspector,}=this.state;
        return (
            <div className='page'>
                ...
                <div className='page-content' style={{padding: 0,background: 'transparent'}} >
                            ...
                            <Row gutter={46}>
                                <Col span={6}>
                                    <FormItem  {...formItemLayout}>
                                          <WeekPicker  style={{ width: '100%' }} placeholder="选择周" onChange={this.onDateChange.bind(this)}/>
                                    </FormItem>
                                </Col>
                                <Col span={6}>
                    ...
                    <Spin tip="加载中.." spinning={loading}>
                        ...
                        >
                            <Row gutter={50}>
                                <Col span={24}>
                                    <label>本周注册人数合计<nobr {...numberStyle}>{userProfile.userSum}</nobr>人，较上周({userProfile.LastWeek}人){this.thanSize(userProfile.userSum,userProfile.LastWeek)}。其中，手机APP注册<nobr {...numberStyle} >{userProfile.userApp}</nobr>人，微信注册<nobr {...numberStyle} >{userProfile.userWechat}</nobr>人，支付宝注册<nobr {...numberStyle} >{userProfile.userAlipay}</nobr>人</label>
                                </Col>
                                <Col span={7}></Col>
                                <Col span={8}>
                        ...
                        >
                            <Row gutter={50}>
                                <Col span={24}>
                                    <label>本周泊位平均占用率为<nobr {...numberStyle} >{berthSituation.occupancyRate}</nobr>，较上周（{berthSituation.LastWeek}）{this.thanSize(berthSituation.occupancyRate,berthSituation.LastWeek)}。其中，{berthSituation.dateMax}平均占用率最高（<nobr {...numberStyle}>{berthSituation.dateMaxNum}</nobr>），{berthSituation.dateMin}日平均占用率最低（<nobr {...numberStyle}>{berthSituation.dateMinNum}</nobr>）。</label>
                                </Col>
                                <Col span={24}>
                                          <div style={{textAlign:'center'}}>
                                ...
                                <Col span={24}>
                                    <label>本周泊位平均周转率为<nobr {...numberStyle} >{berthSituation.occupancyRate2}</nobr>，较上周（{berthSituation.LastWeek2}）{this.thanSize(berthSituation.occupancyRate2,berthSituation.LastWeek2)}。其中，{berthSituation.dateMax2}周转率最高（<nobr {...numberStyle}>{berthSituation.dateMaxNum2}</nobr>），{berthSituation.dateMin2}周转率最低（<nobr {...numberStyle}>{berthSituation.dateMinNum2}</nobr>）。</label>
                                </Col>
                                <Col span={24}>
                                          <div style={{textAlign:'center'}}>
                        ...
                        >
                            <Row gutter={50}>
                                <Col span={24}>
                                    <label>本周停车<nobr {...numberStyle} >{parkingProfile.parkingSum}</nobr>次，较上周（{parkingProfile.LastWeek}次）{this.thanSize(parkingProfile.parkingSum,parkingProfile.LastWeek)}。其中，0-1小时<nobr {...numberStyle} >{parkingProfile.parking1}</nobr>次，0.5-2小时<nobr {...numberStyle} >{parkingProfile.parking2}</nobr>次，2-4小时<nobr {...numberStyle} >{parkingProfile.parking4}</nobr>次，4-8小时<nobr {...numberStyle} >{parkingProfile.parking8}</nobr>次，大于8小时<nobr {...numberStyle} >{parkingProfile.parkingmax}</nobr>次，平均停车时长<nobr {...numberStyle} >{parkingProfile.parkingAverage}</nobr>分钟</label>
                                </Col>
                                <Col span={7}></Col>
                                <Col span={8}>
                                          <div style={{textAlign:'center'}}>
                        ...
                        >
                            <Row gutter={50}>
                                <Col span={24}>
                                    <label>本周缴费笔数<nobr {...numberStyle} >{spendingProfile.strokeCount1}</nobr>笔,较上周({spendingProfile.LastWeek}笔){this.thanSize(spendingProfile.strokeCount1,spendingProfile.LastWeek)}。其中，手机APP缴费<nobr {...numberStyle} >{spendingProfile.strokeCount2}</nobr>笔，微信缴费<nobr {...numberStyle} >{spendingProfile.strokeCount3}</nobr>笔，支付宝缴费<nobr {...numberStyle} >{spendingProfile.strokeCount4}</nobr>笔。</label>
                                </Col>
                                <Col span={24}>
                                    <label>本周缴费金额<nobr {...numberStyle} >{spendingProfile.money1}</nobr>元,较上周({spendingProfile.LastWeek2}元){this.thanSize(spendingProfile.money1,spendingProfile.LastWeek2)}。其中，手机APP缴费<nobr {...numberStyle} >{spendingProfile.money2}</nobr>元，微信缴费<nobr {...numberStyle} >{spendingProfile.money3}</nobr>元，支付宝缴费<nobr {...numberStyle} >{spendingProfile.money4}</nobr>元，平均缴费金额<nobr {...numberStyle} >{spendingProfile.money5}</nobr>元。</label>
                                </Col>
                                <Col span={2}></Col>
                                <Col span={9}>
                                          <div style={{textAlign:'center'}}>
