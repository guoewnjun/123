...
export default class ParkingRecordDetail extends Component {
    ...
    componentDidMount () {
        if (window.checkPageEnable('parkingRecordDetail')) {
            ...
            if (!this.state.orderId) {
                ...
                window.location.hash = 'OperatingCenter/DealManage/ParkingRecord/ParkingRecordDetail?id=' + orderId;
                this.setState({
                    orderId
                })
            } else {
    ...
    checkAppealDetail () {
        if (this.isFromAppealDetail()) {
            ...
            location.hash = `${location.hash.split('?')[0]}/AppealDetail?orderId=${this.props.location.query.id}`;
        }
    }
